...
I've been experimenting with various Business Intelligence tools recently and wanted to try [Redash](https://redash.io/). I have a MySQL database on a Laravel Forge-provisioned server that I'd like to analyze using Redash. However, the server only allows connections via SSH, and Redash doesn't provide built-in SSH tunneling for MySQL connections. So, I had to do a few things to get this set up properly.
